...
public class VejstykkeQueryParams : BaseDawaRequest
{
    ...
    public override Dictionary<string, string?> ToQueryParameters()
    {
        ...
        if (Kode is not null) dict["kode"] = Kode.Value.ToString();
        if (Navngivenvej_id is not null) dict["navngivenvej_id"] = Navngivenvej_id.Value.ToString();
